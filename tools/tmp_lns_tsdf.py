...
SCANNET_ROOT = Path('/home/rzhu/Documents/data/finerecon_data/scannet_tsdf')
...
for split in ['train', 'val', 'test']:
    ...
    for scene_name in tqdm(scene_names):
        if split in ['train', 'val']:
            scene_path = SCANNET_ROOT / 'scans' / (scene_name + '.npz')
        else:
            scene_path = SCANNET_ROOT / 'scans_test' / (scene_name + '.npz')
        assert scene_path.exists(), f'{scene_path} does not exist'
        
        new_scene_path = SCANNET_ROOT / (scene_name + '.npz')
